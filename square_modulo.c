...
int2048 Square_Multiply(int2048 x,int2048 c,int2048 n);
...
int main()
{
	...
	printf("Enter The 1st Positive Integer a in hex:\n");
	scanf("%512s",str1);
	store(&x,str1);

	printf("Enter The 2nd Positive Integer c in hex:\n");
	scanf("%512s",str2);
	store(&c,str2);

	printf("Enter The 3rd Integer b in hex:\n");
	scanf("%512s",str3);
	store(&n,str3);

	z=Square_Multiply(x,c,n);
...
int2048 Square_Multiply(int2048 x,int2048 c,int2048 n)
{
	...
	while(i>=0)
	{
		...
		if((right_shift(c,i).a[31] & 1UL) == 1UL)
		{
			a=multi(a,x);
			a=modulo(a,n);
		}
