...
int main()
{
	...
	printf("Enter The 1st Integer n in hex:\n");
	scanf("%512s",str1);
	store(&x,str1);

	printf("Enter The 2nd Integer a Of Which You want Inverse in hex:\n");
	scanf("%512s",str2);
	store(&y,str2);
...
int2048 inverse(int2048 a,int2048 n)
{
	
	int2048 v0,v1,u0,u1,q,a0,b0,r0,r1,r,temp;
	v0=ONE;
	v1=ZERO;
	u0=ZERO;
	u1=ONE;
	...
	while(compare(n,ZERO) == 2)
	{

		q=division(a,n);
		r=sum(a,multi(q,n),1);

		temp=sum(u0,multi(u1,q),1);
		u0=u1;
		u1=temp;

		temp=sum(v0,multi(v1,q),1);
		v0=v1;
		v1=temp;
